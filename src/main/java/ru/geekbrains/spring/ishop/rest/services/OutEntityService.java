...
//        }
        //with this
        initConverter(deserializersBeans, deserializers, Converters.deserializer.name());
...
//        }
        //with this
        initConverter(serializersBeans, serializers, Converters.serializer.name());

        serializerFactory.initSerializerFactory(serializers);
        outEntitySerializer.setSerializerFactory(serializerFactory);
    }

    private void initConverter(Map<String, ? extends IEntityConverter> convertersBeans,
                               Map<String, ? extends IEntityConverter> converters,
                               String converterType) {
        Set<String> keys = convertersBeans.keySet();
        for (String key : keys) {
            for (EntityTypes type : EntityTypes.values()) {
                String modifiedKey = key.toLowerCase().replace(converterType, "");
                if(modifiedKey.equals(type.name().toLowerCase())) {
                    converters.put(type.name(), convertersBeans.get(key));
                    break;
                }
            }
        }
